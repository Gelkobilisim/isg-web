...
app.post("/api/login", async (req, res) => {
  ...
  try {
    ...
    if (usersSnapshot.empty) {
      return res.status(401).json({ error: `Geçersiz kullanıcı adı: ${username} sistemde bulunamadı.` });
    }
    ...
    if (!secretDoc.exists) {
      return res.status(401).json({ error: `Güvenlik hatası: ${username} için şifre tablosu (user_secrets) bulunamadı.` });
    }
    
    if (secretDoc.data().password !== password) {
